...
function verifyProperties(user) {
  return new Promise((resolve, reject) => {
    ...
    if (user.username && user.password) resolve(user);
    else reject({ code: 400, message: "Invalid model" });
  });
}

// Funcion para verificar que el ID del usuario sea el correcto
function verifyId(user) {
  return new Promise((resolve, reject) => {
    if (
      typeof user._id === "string" &&
      user._id.length == 24 &&
      user._id.match("^[0-9A-Fa-f]+$")
    ) {
      user._id = new ObjectId(user._id);
      resolve(user);
    } else reject({ code: 400, message: "Please provide a valid ID" });
  });
}
...
//Funcion para notificar que no hay contenido
